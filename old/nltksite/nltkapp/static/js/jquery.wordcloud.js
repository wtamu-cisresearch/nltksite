/*!
 ...
 available settings
	...
	abort: abort handler.
	wordPlacementAbort: abort and execute abort() when unable to place a word. true or false. Default is false.
	weightFactor: 
...
(function ($) {
	...
	$.fn.wordCloud = function (options) {
		...
		var g = settings.gridSize,
			...
			updateWordMap = function (gx, gy, w, h, word, weight, ctx) {
				pixelMap[word] = {
					x: gx,
					y: gy,
					width: w,
					...
					if (points.shuffle().some(
						function (gxy) {
							if (canFitText(gxy[0], gxy[1], gw, gh)) {
								...
								updateWordMap(gxy[0]*g + (gw*g - w)/2, gxy[1]*g + (gh*g - h)/w, w, h, word, weight, ctx);
								if (settings.fillBox) {
			...
			if (settings.wait !== 0) {
				var timer = setInterval(
					function () {
						...
						var wordPlaced = putWord(settings.wordList[i].word, settings.wordList[i].freq);
						if (!wordPlaced) {
							console.log("Failed to place word: ", settings.wordList[i].word, " at font size: ", settings.weightFactor(settings.wordList[i].freq));
						}
						if (exceedTime() || (settings.wordPlacementAbort == true && !wordPlaced)) {
							console.log("Aborting word cloud.");
							clearTimeout(timer);
							settings.abort();
				...
				window.setImmediate(
					function loop() {
						...
						escapeTime = (new Date()).getTime();
						$("#wordcloudinfo").html("Total Words: " + settings.wordList.length + ". Attempting to put word: " + (i+1) + " " + settings.wordList[i].word);
						// Only try to place the word if we aren't excluding it
						var abort = false;
						if (settings.wordList[i].exclude == 0) {
							if (!putWord(settings.wordList[i].word, settings.wordList[i].freq)) {
								console.log("Failed to put word: ", settings.wordList[i].word, " at font size: ", settings.weightFactor(settings.wordList[i].freq));
								if (settings.wordPlacementAbort == true) {
									abort = true;
								} else {				
									wordList[i].exclude = 1; // exclude code 1 means Not enough room in wordcloud
								}
							}
						}
