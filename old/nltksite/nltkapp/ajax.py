...
@dajaxice_register
def get_documents(request, corpus_id):
	...
	return json.dumps({'documents': documents_list})

@dajaxice_register
def get_sentences(request, corpus_id, document_ids, word):
	...
	return json.dumps({'word': word, 'wdmatrix':finalResult})
	
@dajaxice_register
def wordfreq(request, corpus_id, document_ids, ngram, scoring_method):
	...
	return json.dumps({'list':word_list})
...
@dajaxice_register
def wordnet_data(request, words):
	results = []
	for w in words:
...
def open_corpus(corpus_id, document_ids):
	...
	return PlaintextCorpusReader(c.get_path(), fileids), ""